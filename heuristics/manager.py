...
# heuristics/manager.py
# Complexity Router — empirically tuned parameters.
#
# Parameter scaling based on actual benchmark results:
#   - Generations drive quality more than population
#   - Population 20-30 sufficient; 40+ adds runtime without gain
#   - SA sweet spot: converges around gen 130-150
#   - Hybrid SA cooling 0.97 slightly better than 0.95
#
# Gate count tiers (from your circuit list):
#   Tiny   : <300    (c17=6, s820=292, s832=294)
#   Small  : 300-600 (c880=383, s953=424, s1196=547, s1238=526)
#   Medium : 600-1500 (s1488=659, s1494=653, c1355=546, c1908=880)
#   Large  : 1500-5000 (c2670=1193, c3540=1669, s5378=2574, s13207=2107)
#   XLarge : >5000   (c7552=3512, s15850, s35932, s38584=20679)
...
# Based on actual s1196 benchmark results.
# ─────────────────────────────────────────────────────────────

def scale_iterations(gate_count: int) -> int:
    """
    SA iterations per temperature step.
    Scales so each gate gets ~touched proportionally.
    """
    return min(200, max(10, gate_count // 50))
...
def scale_ga_params(gate_count: int) -> dict:
    """
    GA parameters empirically tuned.

    Key insight from benchmarks:
      - generations=130-150 is sweet spot (converges there anyway)
      - population=20-30 sufficient
      - higher population just slows without gain

    Tiers:
      Tiny   <300      : pop=20, gen=80   (fast, small space)
      Small  300-600   : pop=25, gen=140  (sweet spot range)
      Medium 600-1500  : pop=25, gen=150  (needs more exploration)
      Large  1500-5000 : pop=30, gen=160  (diminishing returns)
      XLarge >5000     : pop=30, gen=150  (keep runtime sane)
    """
    if gate_count < 300:
        population, generations = 20, 80
    elif gate_count < 600:
        population, generations = 25, 140
    elif gate_count < 1500:
        population, generations = 25, 150
    elif gate_count < 5000:
        population, generations = 30, 160
    else:
        population, generations = 30, 150
...
def build_configs(gate_count: int) -> dict:
    ...
    return {
        ...
        "ga": ga_cfg,
        # Hybrid: GA explores then SA refines with 0.97 cooling
        # (empirically 0.97 > 0.95 for hybrid SA phase)
        "hybrid": dict(
            ga_population   = ga_cfg['population_size'],
            ga_generations  = ga_cfg['generations'],
            ga_survival     = 0.3,
...
# ─────────────────────────────────────────────────────────────
# GNN VALIDITY RANGE
# Updated after multi-circuit retraining.
# ─────────────────────────────────────────────────────────────
...
def optimize_all(circuit, verbose: bool = True) -> dict:
    ...
    Returns best result and writes output files.
    """
    gate_count = circuit.gate_count
    configs    = build_configs(gate_count)
    ga_params  = scale_ga_params(gate_count)
    ...
    def _run(label, fn, *args, **kwargs):
        print(f"  [{label:<7}] running...", end=" ", flush=True)
        t0            = time.perf_counter()
...
# ─────────────────────────────────────────────────────────────
# SINGLE OPTIMIZER ENTRY POINT (API)
# ─────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    print("Parameter scaling table:")
    print(f"  {'Gates':>8}  {'Pop':>5}  {'Gen':>5}  {'SA_iters':>10}")
    print(f"  {'-'*8}  {'-'*5}  {'-'*5}  {'-'*10}")
    for n in [6, 292, 383, 547, 659, 1193, 2574, 7552, 20679]:
        p = scale_ga_params(n)
